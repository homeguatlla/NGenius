...
MouseToEventBind::MouseToEventBind(int button, int action, GameEvent* event) :
	mButton(button), 
	mAction(action),
	mEvent(event)
{
...
MouseToEventBind::MouseToEventBind(int button, GameEvent* event) :
	mButton(button),
	mAction(-1),
	mEvent(event)
{
...
GameEvent* MouseToEventBind::Convert(const void* data) const
{
